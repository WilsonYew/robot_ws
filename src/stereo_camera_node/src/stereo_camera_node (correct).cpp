...
public:
  StereoCameraNode() : Node("stereo_camera_node")
  {
    ...
    this->declare_parameter<std::string>("encoding", "mono8"); // kept for compatibility
    this->declare_parameter<std::string>("maps_yaml", "src/stereo_camera_node/config/stereo_rectify_maps.yml");
    this->declare_parameter<std::string>("intrinsics_yaml", "");
    // Frame IDs – REP-103 optical frames
    ...
    this->get_parameter("encoding", encoding_); // not used for rect topics anymore
    ...
    left_pub_       = this->create_publisher<sensor_msgs::msg::Image>(left_topic_, sensor_qos);
    right_pub_      = this->create_publisher<sensor_msgs::msg::Image>(right_topic_, sensor_qos);
    left_info_pub_  = this->create_publisher<sensor_msgs::msg::CameraInfo>(left_info_topic_, sensor_qos);
    right_info_pub_ = this->create_publisher<sensor_msgs::msg::CameraInfo>(right_info_topic_, sensor_qos);
    left_rgb_pub_   = this->create_publisher<sensor_msgs::msg::Image>("/left/image_rect_rgb", sensor_qos);

    if (left_topic_ == right_topic_) {
      RCLCPP_WARN(get_logger(), "left_topic and right_topic are identical (%s). One side will be hidden.",
                  left_topic_.c_str());
    }
    RCLCPP_INFO(get_logger(), "Topics:\n  left : %s (mono8)\n  right: %s (mono8)\n  left_rgb: /left/image_rect_rgb (bgr8)",
    ...
    // Prefer color from the driver; OpenCV will convert to BGR.
...
private:
  ...
  rclcpp::Publisher<sensor_msgs::msg::Image>::SharedPtr left_rgb_pub_;
  ...
  bool loadIntrinsicsFromYaml(const std::string &yaml_path)
  {
    ...
    if (P_l.empty() || P_r.empty() || P_l.cols != 4 || P_r.cols != 4) {
      RCLCPP_WARN(get_logger(), "No valid P1/P2 in YAML; using fx/fy/cx/cy + baseline params.");
      return false;
    }
    ...
    camera_info_right_.p[3] = Tx; // contains -fx*B from YAML

    fx_ = fx; fy_ = fy; cx_ = cx; cy_ = cy;
    if (fx_ != 0.0) baseline_ = -Tx / fx_;
    return true;
  }
  ...
  void tick()
  {
    ...
    // Force BGR source for both eyes
    if (frame_left.channels() == 1)  cv::cvtColor(frame_left,  frame_left,  cv::COLOR_GRAY2BGR);
    if (frame_right.channels() == 1) cv::cvtColor(frame_right, frame_right, cv::COLOR_GRAY2BGR);

    // Ensure sizes match rectification maps
    ...
    // Rectify in BGR
    cv::Mat left_rect_bgr, right_rect_bgr;
    cv::remap(frame_left,  left_rect_bgr,  left_map1_,  left_map2_,  cv::INTER_LINEAR);
    cv::remap(frame_right, right_rect_bgr, right_map1_, right_map2_, cv::INTER_LINEAR);

    // Prepare headers
    std_msgs::msg::Header hdr_left, hdr_right;
    hdr_left.stamp = this->now();
    hdr_right.stamp = hdr_left.stamp;
    hdr_left.frame_id = left_frame_id_;
    hdr_right.frame_id = right_frame_id_;

    // Convert rectified BGR → MONO for the main rect topics
    cv::Mat left_rect_mono, right_rect_mono;
    cv::cvtColor(left_rect_bgr,  left_rect_mono,  cv::COLOR_BGR2GRAY);
    cv::cvtColor(right_rect_bgr, right_rect_mono, cv::COLOR_BGR2GRAY);

    // Publish main left/right as mono8
    auto left_msg  = cv_bridge::CvImage(hdr_left,  "mono8", left_rect_mono).toImageMsg();
    auto right_msg = cv_bridge::CvImage(hdr_right, "mono8", right_rect_mono).toImageMsg();
    ...
    // Publish left_rect_rgb as bgr8 for YOLO
    auto left_rgb_msg = cv_bridge::CvImage(left_msg->header, "bgr8", left_rect_bgr).toImageMsg();
    left_rgb_pub_->publish(*left_rgb_msg);

    // Diagnostics
    RCLCPP_INFO_THROTTLE(get_logger(), *this->get_clock(), 2000,
      "PUB: %s (mono8 %ux%u) | %s (mono8 %ux%u) | /left/image_rect_rgb (bgr8 %ux%u)",
      left_topic_.c_str(),  left_msg->width,  left_msg->height,
      right_topic_.c_str(), right_msg->width, right_msg->height,
      left_rgb_msg->width, left_rgb_msg->height);
  }
};
